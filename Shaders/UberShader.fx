...
float3 CalculateSpecularBlinn(float3 viewDirection, float3 normal, float2 texCoord)
{
	...
	// Calculate Blinn
	float blinn = saturate(dot(halfVector, normal));
	blinn = pow(blinn, gShininess);
	
	// Apply properties
	specularColor *= blinn * specularIntensity;
	blinn *= specularIntensity;
...
float3 CalculateSpecularPhong(float3 viewDirection, float3 normal, float2 texCoord)
{
	...
	float phong = dot(viewDirection, reflectDir);
	phong = saturate(phong);
	phong = pow(phong, gShininess);
	
	// Apply properties
	specularColor *= phong * specularIntensity;
...
{
	float3 fresnelResult = float3(1.f,1.f,1.f);
	
	if (gUseFresnelFalloff)
	{
		fresnelResult = gColorFresnel;
		
		// Fresnel falloff
		float fresnel = dot(normal, viewDirection);
		fresnel = saturate(abs(fresnel));
		float inverseFresnel = 1 - fresnel;
		fresnel = saturate(pow(inverseFresnel, gFresnelPower));
		...
		fresnelResult *= fresnel * fresnelMask;
	}
	
	return environmentColor * fresnelResult;
}

float3 CalculateEnvironment(float3 viewDirection, float3 normal)
{
	float3 environmentMap = float3(0, 0, 0);
	
	if (gUseEnvironmentMapping)
	{
		float3 cameraPosition = gMatrixViewInverse[3].xyz;
		float3 reflectedVector = reflect(viewDirection, normal);
		float3 refractedVector = refract(viewDirection, normal, gRefractionIndex);
		
		float3 reflectedSample = gCubeEnvironment.Sample(gTextureSampler, reflectedVector) * gReflectionStrength;
		float3 refractedSample = gCubeEnvironment.Sample(gTextureSampler, refractedVector) * gRefractionStrength;
		
		environmentMap = reflectedSample + refractedSample;
	}
